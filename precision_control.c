...
int precision_control(wstate_struct* ws, cstate_struct* cs, nstate_struct* ns, soilprop_struct* sprop, soilInfo_struct* soilInfo)
{
	...
	cpool = cs->leafc;
	npool = ns->leafn;
	precision_reset(1, 0, cs, ns, &cpool, &npool, &errorCode);
	cs->leafc = cpool;
	ns->leafn = npool;
	...
	for (layer = 0; layer < N_SOILLAYERS; layer++)
	{
		...
		precision_reset(1, 1, cs, ns, &cpool, &npool, &errorCode);
	...
	{
		printf("ERROR in check_soilcontent.c for groundwater_preproc.c\n");
		errorCode = 1;
	}

	if (GWlayer > DATA_GAP && GWlayer < N_SOILLAYERS)
	{
		for (dm = 0; dm < N_DISSOLVMATER; dm++)
		{
			if (soilInfo->content_soil[dm][CFlayer] == 0)
			{
				soilInfo->content_CAPILcf[dm] = 0;
				soilInfo->content_NORMcf[dm] = 0;
			}
			if (soilInfo->content_soil[dm][GWlayer] == 0)
			{
...
int precision_reset(int flagLITTER, int layer, cstate_struct* cs, nstate_struct* ns, double* cpool, double* npool, int* errorCode)
{
	...
	double CRIT_PREC1, CRIT_PREC2, CRIT_PREC3, litrClayer;

	/* setting critical precision */
	if (flagLITTER == 0)
	{
		CRIT_PREC1 = CRIT_PREC;
		CRIT_PREC2 = CRIT_PREC;
	}
	else
	{
		CRIT_PREC1 = CRIT_PREC;
		CRIT_PREC2 = CRIT_PREC_RIG;
	}
	CRIT_PREC3 = CRIT_PREC_lenient;

	/* examination of cpool and npool values */
	if ((fabs(*cpool) < CRIT_PREC1 && *cpool != 0) || (fabs(*npool) < CRIT_PREC2 && *npool != 0))
	{
		if (fabs(*cpool) > CRIT_PREC3 || fabs(*npool) > CRIT_PREC3)
		{
			printf("\n");
			printf("ERROR: irrealistic C:N ratio in plant/litter/soil material (precision_control.c)\n");
			*errorCode = 1;
		}
		else
		{
